...
describe('Phase1 Invariants: rapid browse/switch interleaving', () => {
  let sm;

  beforeEach(() => {
    sm = createStateManager('proj-a');
    globalThis.StateManager = sm;
    globalThis.ContextHealthBar = { update: vi.fn() };
    globalThis.ToolDock = { updateForProject: vi.fn() };
    globalThis.KnowledgeMap = { refresh: vi.fn() };
  ...
  it('only switchProject triggers websocket reconnect and final state matches last action', async () => {
    sm.browseProject('proj-b');
    sm.browseProject('proj-c');
    await sm.switchProject('proj-d');
    sm.browseProject('proj-e');
    await sm.switchProject('proj-f');

    expect(sm.activeProjectId).toBe('proj-f');
    expect(sm._calls.connectWebSocket).toHaveLength(2);
    expect(ToolDock.updateForProject).toHaveBeenCalledWith('proj-f');
    expect(KnowledgeMap.refresh).toHaveBeenCalledWith('proj-f');
  });

  it('sendMessage after interleaving targets final project context', async () => {
    const sent = [];
    sm._state.socket = { readyState: 1, send: (raw) => sent.push(JSON.parse(raw)) };

    sm.browseProject('proj-b');
    await sm.switchProject('proj-c');
    sm.browseProject('proj-d');
    await sm.switchProject('proj-e');
    sm.sendMessage('hello final context');

    expect(sent).toHaveLength(1);
    expect(sent[0].project_id).toBe('proj-e');
    expect(sent[0].message).toBe('hello final context');
  });

  it('deterministic long interleaving keeps websocket reconnects equal to switch count', async () => {
    const sent = [];
    sm._state.socket = { readyState: 1, send: (raw) => sent.push(JSON.parse(raw)) };
    ...
    let switchCount = 0;
    let lastProject = sm.activeProjectId;
    for (const [kind, projectId] of actions) {
      if (kind === 'browse') {
    ...
    sm.sendMessage('long-run final project check');

    expect(sm.activeProjectId).toBe(lastProject);
    expect(sm._calls.connectWebSocket).toHaveLength(switchCount);
    expect(sent).toHaveLength(1);
    expect(sent[0].project_id).toBe(lastProject);
  });

  it('deterministic 3-project sweep preserves route order and message project context', async () => {
    const sent = [];
    sm._state.socket = { readyState: 1, send: (raw) => sent.push(JSON.parse(raw)) };

    // Deterministic interleaving where each switch targets a different active project.
    const route = [
      ['browse', 'proj-b'],
      ['switch', 'proj-c'],
      ['browse', 'proj-b'],
    ...
    const switchedTo = [];
    for (const [kind, pid] of route) {
      if (kind === 'browse') {
        sm.browseProject(pid);
      } else {
        await sm.switchProject(pid);
        switchedTo.push(pid);
      }
    }

    expect(switchedTo).toEqual(['proj-c', 'proj-a', 'proj-b', 'proj-c']);
    expect(sm._calls.connectWebSocket).toHaveLength(switchedTo.length);
    expect(sm.activeProjectId).toBe('proj-c');

    // Final send must bind to final active project.
    sm.sendMessage('deterministic sweep final');
    expect(sent).toHaveLength(1);
    expect(sent[0].message).toBe('deterministic sweep final');
    expect(sent[0].project_id).toBe('proj-c');
  });
});
