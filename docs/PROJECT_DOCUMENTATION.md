...
| `memory_write` | `src/agent/nodes/memory.py` | Extracts topics/interests, saves to Mem0/ChromaDB, invalidates cache. |
...
**Long-Term:** Mem0 + ChromaDB (multilingual-e5-small embeddings). Project-scoped isolation: non-default projects use `project:<id>` as user_id.
...
| ChromaDB | `chromadb/chroma:latest` | 8100 | Long-term memory vector store |
...
├── docker-compose.yml            # Redis + ChromaDB + SearXNG
