...
| Small LLM | Liquid LFM2.5-1.2B (~700 tok/s routing) |
| Medium LLM | Qwen3.5-9B (reasoning + tool calling, swappable variants) |
...
src/memory/          Memory system
  ...
  └── long_term.py          Mem0 + ChromaDB integration
...
- **Docker/Podman** for ChromaDB and SearXNG containers
...
# - Small: liquid/lfm2.5-1.2b
# - Medium: qwen/qwen3.5-9b

# 4. Start services + backend + desktop app
./start.sh

# Or start just the backend:
# docker-compose up -d   # ChromaDB + SearXNG + Redis
